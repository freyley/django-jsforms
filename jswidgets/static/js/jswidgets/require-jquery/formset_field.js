define([], function() {
    ...
    var renumber_form = function(form, number) {
        console.log("renumbering", form, number);
    };
    ...
    var delete_form = function(form, data) {
        ...
        var id_field = form.find("#id_jswidgets-book_formats-" + form.number + "-id");
        if(id_field.val() == "") {
            // form was created on this page
            form.remove();
            var total = 0;
            $.each($(".jswidgets-formsetfield-form-" + data.name), function(_, f) {
                renumber_form(f, total);
                total += 1;
            });
